...
To execute this project, I will utilize part of the datasets and queries developed in [Phase 1](./README.md), including the data structure and organization. We will use the datasets that have already been prepared, validated, and cleaned through a **Python script**, which can be viewed [here](./Partner_Performance_Insights_Business_Case.ipynb). These datasets will serve as the information source; we will not import them into SSMS but will use the already structured data to validate our automation process.
...
The step-by-step guide for building the database can be viewed [here](./SQL%20Documentation.md).
...
Create the tables within the database: [Link](./Assets/SQL%20Queries/q2_table_created_ofc.sql)

Register the partners: [Link](./Assets/SQL%20Queries/insert_partners.sql)

Simulate transactions and visits: [Link](./Assets/SQL%20Queries/insert_payment_transactions.sql)

Change requests for CAP and cost: [Link](./Assets/SQL%20Queries/insert_cap_change_log.sql)

Complete dataset, which feeds Google Sheets and Looker Studio: [Link](./Assets/SQL%20Queries/q2_fullbase_ofc.sql)
